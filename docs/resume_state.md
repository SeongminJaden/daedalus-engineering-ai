...
| stage | state |
|---|---|
| specification and arm | `projects/manipulator/spec.py`, `arm.py`; reach 600 mm exactly, gravity along -y |
| policy, dynamics, drivetrain, inertia | run; two joints CANNOT be driven from the sourced catalogue |
| mass torque loop | converges in 3 iterations, structure 2.21 to 0.914 kg, shoulder torque 22.2 to 27.6 N m |
| link design, both paths | both run on the upper arm and forearm, comparability judged per link |
| verification, fatigue, features, manufacturability, assembly | run; Gazebo statics agrees to 0.03 percent on six joints |
| document | `docs/manipulator_design.md`, generated by `scripts/design_manipulator.py` |
| drive pool | 14 sourced actuators and 7 gear units across ROBOTIS, CubeMars, Kollmorgen, maxon, mjbots, DAMIAO, Apex, Harmonic Drive, Nabtesco; grades and bus voltages stored |
| bus voltage | a design decision with a mass consequence: 4.347 kg of drives at 24 V, 3.760 at 36 V, 3.103 at 48 V |
| envelope | actuator outlines are now a constraint: wrist spacing 40 mm from the 38.5 mm drive, sections floored at the 98 mm drive diameter; structure 1.204 kg, total 4.947 kg |
| not done | covers, wiring, bearings beyond a seat tolerance, bolted joint analysis, shoulder bracket |
