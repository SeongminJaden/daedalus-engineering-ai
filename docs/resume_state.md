...
Every link is now a generated shape with drawing-backed fastening, and the
two parts that hold the arm to the world are designed. A Fusion session on
another machine stands the parts up and measures them, and four defects in
this list were found that way rather than by any check here.

| stage | state |
|---|---|
| specification and arm | `projects/manipulator/spec.py`, `arm.py`; reach 600 mm exactly, gravity along -y, payload a stated 100 mm cube |
| kinematics | j2 origin_y 150, j3 origin_x 196.27, j4 168.23, j5 78.5, j6 78.5 mm. The wrist spacing grew from 68.5 to 78.5 when 10 mm of assembly clearance was demanded |
| material | `alsi10mg_slm`, not 6061. The links are powder bed parts and were being sized in a wrought alloy. Direction resolved tables for two machines in `core/materials/printed.py` |
| mass torque loop | converges upward in 3 iterations, 3.09 to 4.80 kg structure. It used to converge downward and the test said so; the wrist clearance and the flanges changed that |
| drive selection | a candidate with no published drawing is refused however light it is. That rule replaced the frameless motor at the tool roll with an AK60-6 |
| mounting interfaces | `projects/manipulator/interfaces.py`, read off CubeMars 2D drawings and their own 3D models. Bolt circles, thread depths, face insets, clock angles, dowels, and what each drawing does NOT print |
| links | `projects/manipulator/links.py`, `scripts/generate_links.py`. Six bodies, millimetres, watertight, with clearance holes, central bores and dowels cut after extraction |
| mounts | `projects/manipulator/mounts.py`, `scripts/generate_mounts.py`. Base mount 1.372 kg, tool plate 0.576 kg |
| spigot and friction | `spigot_stage`: no friction coefficient is assumed, the coefficient the joint NEEDS is reported instead. Worst joint 0.027; dowel bearing 66.3 N m against a 22.6 N m shoulder peak |
| document | `docs/manipulator_design.md`, generated by `scripts/design_manipulator.py` |
| not done | covers, wiring, bearings beyond a seat tolerance, the spigot fit itself (no boss tolerance is printed), the volume fraction search |

Four defects found by standing the parts up, none of which any check here
could see:

- The extracted body sat half an element low on all three axes, 4.08 mm
  across and 3.5 along. Volume is invariant under translation, so every
  volume check passed.
- The extracted triangles pointed inward. Every volume here is read through
  `abs()`, so nothing noticed until a mesh boolean refused the body.
- The actuator pocket was not on the joint axis, the base column's axis was
  ...
  that touch a link was cut for.
- Keeping the largest connected component silently discarded a slab that was
  held solid for an interface, which shortened the tool flange by 23 mm. That
  case is now a refusal.
