...
**토폴로지 결과는 설계 개념이고, 부품과 얼마나 먼지는 이제 측정값이다.** SIMP는 임계처리해야 하는 중간 밀도를 남기므로 얻는 형상은 최적화한 밀도장과 다르다. 추출한 부품을 CalculiX 로 다시 풀어 밀도장과 비교했다(`docs/topology_design.md`): 기본 실행에서는 하중이 걸리는 요소가 회색으로 남아 임계처리가 하중 경로를 끊었고, 그래서 `SimpProblem` 이 하중·지지 패시브 영역을 갖게 됐다. 3필드 투영과 충분한 해상도에서는 회색 비율이 2 퍼센트로 떨어지고 추출 부품이 모든 임계값에서 밀도장의 3 퍼센트 이내로 들어온다. 여전히 깨끗한 STEP 이 아니라 각진 복셀 STL 로 나오고, **응력이 아니라 컴플라이언스**를 최소화하며 최대응력에 대해 아무 말도 하지 않는다. 3D FEM 관문을 여전히 통과해야 한다.
...
| P9 | 에이전트 루프가 `generative_cad` 전략을 실행: 문제 길이를 고정한 11 패밀리 후보를 5 하중 케이스 중 하나로, 등방성 재료는 정확한 스케일링으로 훑고 승자는 자기 재료로 다시 해석, 프록시 또는 형상 서로게이트가 순위 매기고 CalculiX 가 검증, 제조성 규칙은 선호축이며 판정 아님, 승자의 STEP 경로가 에피소드에 기록; 자유형 아님을 문서에 명시 | 완료(이 범위까지) |
...
| 데이터셋 | 패밀리 13, 하중 케이스 5(각각 닫힌 형태 검증), 케이스별 측정한 Poisson 잔차를 단 정확한 재료 스케일링, 재개 가능한 배치 생성; 사양과 비용은 `docs/dataset_spec.md` | 완료; 1차 생성 2026-09-03: 해석 부품 6,430, 스케일링 포함 113,152 레코드, 수치는 사양 문서에 |
...
**생성된 데이터셋 위의 서로게이트·임베딩(2026-09-03).** 해석 6,430 부품에서 나온 90,574 행으로
학습하고 22,578 행으로 측정: primary_response 순위 상관 0.997, 중앙 오차 7 퍼센트, p95 38 퍼센트.
같은 행에서 닫힌 형태 프록시 단독은 0.878, 36 퍼센트이고, 이 기준선이 앞 숫자를 의미 있게 만든다.
데이터가 이만큼이면 프록시 피처는 더 이상 필요 없다(빼도 0.997). 40개일 때와 반대다. CAD 임베딩은
13 패밀리에서 precision at 1 이 0.95, 기술자는 0.99. 전부 SURROGATE 이고 아무것도 판정하지 못한다.
